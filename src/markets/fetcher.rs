...
impl FetchProgress {
    ...
    fn display_detailed(&mut self, status: &str) {
        ...
        if let Some(eta) = self.estimated_completion_time {
            ...
            if remaining > 0 {
                print!(" | ETA: {}", format_duration(std::time::Duration::from_secs(remaining as u64)));
            }
        }
...
impl<T: MarketDataProvider> MarketFetcher<T> {
    ...
    fn convert_raw_markets(&self, raw_markets: Vec<serde_json::Value>) -> Result<Vec<Market>> {
                ...
                Ok(market) => {
                    ...
                    match &market.condition_id {
                        ...
                        _ => {
                            ...
                            if self.config.verbose {
                                warn!("Warning: Skipping market with missing/empty condition_id: {}", market.question);
                            }
                        }
                    }
                },
        ...
        if invalid_condition_ids > 0 {
            info!("🔍 Filtered out {} markets with invalid/empty condition_id", invalid_condition_ids);
        }
    ...
    fn display_final_summary(&self, result: &FetchResult) {
        ...
        info!(
            ...
            format_duration(std::time::Duration::from_secs(result.execution_time_seconds as u64)),
        ...
        if result.errors_encountered > 0 {
            warn!("⚠️  {} errors encountered during fetch", result.errors_encountered);
        }
    }
}
