...
use crate::portfolio::{
    Position, ActiveOrder, PortfolioStats, MarketPositionSummary,
...
impl<'a> PortfolioWidget<'a> {
    ...
    fn render_overview(&self, area: Rect, buf: &mut Buffer) {
        ...
        let stats_text = vec![
            ...
            Line::from(vec![
                Span::raw("Available: "),
                Span::styled(format!("${:.2}", self.stats.available_balance), Style::default().fg(Color::Green)),
                Span::raw(" | Locked: "),
                Span::styled(format!("${:.2}", self.stats.locked_balance), Style::default().fg(Color::Gray)),
            ]),
            Line::from(vec![
                Span::raw("Total P&L: "),
                Span::styled(
                    format!("${:.2} ({:+.2}%)", total_pnl, self.stats.pnl_percentage(Decimal::from(10000)).unwrap_or(Decimal::ZERO)),
                    Style::default().fg(pnl_color).add_modifier(Modifier::BOLD)
                ),
            ]),
            Line::from(vec![
                Span::raw("Realized: "),
                Span::styled(format!("${:.2}", self.stats.total_realized_pnl), Style::default().fg(Color::White)),
                Span::raw(" | Unrealized: "),
                Span::styled(format!("${:.2}", self.stats.total_unrealized_pnl), Style::default().fg(Color::Gray)),
            ]),
            Line::from(vec![
                Span::raw("Positions: "),
                Span::styled(format!("{}/{}", self.stats.open_positions, self.stats.total_positions), Style::default().fg(Color::White)),
                Span::raw(" | Win Rate: "),
                Span::styled(
                    self.stats.win_rate.map(|w| format!("{:.1}%", w)).unwrap_or_else(|| "N/A".to_string()),
            ...
            Line::from(vec![
                Span::raw("Avg Win: "),
                Span::styled(
                    self.stats.average_win.map(|w| format!("${:.2}", w)).unwrap_or_else(|| "N/A".to_string()),
                    Style::default().fg(Color::Green)
                ),
                Span::raw(" | Avg Loss: "),
                Span::styled(
                    self.stats.average_loss.map(|l| format!("${:.2}", l)).unwrap_or_else(|| "N/A".to_string()),
        ...
        .header(Row::new(vec!["Type", "Side", "Token", "Size", "Price", "P&L/Status"]))
    ...
    fn render_positions(&self, area: Rect, buf: &mut Buffer) {
        ...
        let header = Row::new(vec![
            "Token", "Side", "Size", "Avg Price", "Current", "P&L", "P&L %", "Status"
        ])
        .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));
        
        let rows: Vec<Row> = self.positions.iter().map(|position| {
            let pnl = position.total_pnl();
            ...
            Row::new(vec![
                ...
                Cell::from(format!("{:.2}", position.size)),
                Cell::from(format!("${:.4}", position.average_price)),
                Cell::from(position.current_price.map(|p| format!("${:.4}", p)).unwrap_or_else(|| "N/A".to_string())),
                Cell::from(format!("${:.2}", pnl)).style(Style::default().fg(pnl_color)),
                Cell::from(format!("{:+.2}%", pnl_pct)).style(Style::default().fg(pnl_color)),
                Cell::from(format!("{:?}", position.status)).style(Style::default().fg(status_color)),
            ])
        }).collect();
    ...
    fn render_orders(&self, area: Rect, buf: &mut Buffer) {
        ...
        let rows: Vec<Row> = self.orders.iter().map(|order| {
            let status_color = match order.status {
                OrderStatus::Open => Color::Yellow,
                OrderStatus::PartiallyFilled => Color::Blue,
                OrderStatus::Filled => Color::Green,
    ...
    fn render_markets(&self, area: Rect, buf: &mut Buffer) {
        ...
        let header = Row::new(vec![
            "Market", "Positions", "Net Pos", "Exposure", "Total P&L", "Orders"
        ])
        .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));
        
        let rows: Vec<Row> = self.market_summaries.iter().map(|summary| {
            let pnl_color = if summary.total_pnl >= Decimal::ZERO { Color::Green } else { Color::Red };
            let net_color = if summary.net_position >= Decimal::ZERO { Color::Green } else { Color::Red };
            
            Row::new(vec![
                Cell::from(format!("{:.40}", summary.market_question)),
                Cell::from(format!("{}", summary.positions.len())),
                Cell::from(format!("{:+.2}", summary.net_position)).style(Style::default().fg(net_color)),
                Cell::from(format!("${:.2}", summary.total_exposure)),
                Cell::from(format!("${:.2}", summary.total_pnl)).style(Style::default().fg(pnl_color)),
