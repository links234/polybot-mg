...
impl SelectionsMenu {
            ...
            if let Event::Key(key) = event::read()? {
                if key.kind == KeyEventKind::Press {
                    match self.mode {
                                ...
                                KeyCode::Enter | KeyCode::Char(' ') => {
                                    if let Some(selected) = self.list_state.selected() {
                                        if let Some(selection) =
                                            self.selections.get(selected).cloned()
                                        {
                                ...
                                KeyCode::Char('e') => {
                                    if let Some(selected) = self.list_state.selected() {
                                        if let Some(selection) = self.selections.get(selected) {
                                            ...
                                            let result =
                                                SelectionBuilder::run(Some(selection.name.clone()))
                                                    .await?;
                        ...
                        _MenuMode::ViewingSelection => match key.code {
                        ...
                        _MenuMode::ConfirmDelete => match key.code {
                            KeyCode::Char('y') => {
    ...
    fn render_selection_details(
        ...
        // Tokens list
        let token_items: Vec<ListItem> = selection
            .tokens
            .iter()
            .enumerate()
            .map(|(i, token)| {
        ...
        let tokens_list = List::new(token_items).block(
            Block::default()
    ...
    fn render_instructions(&self, f: &mut Frame, area: ratatui::prelude::Rect) {
        let instructions = match self.mode {
            ...
            _MenuMode::ViewingSelection => {
                vec![Line::from("Press Enter or Esc to go back")]
            }
            _MenuMode::ConfirmDelete => {
                vec![Line::from("Press 'y' to confirm delete, 'n' to cancel")]
            }
        };
