...
use crate::datasets::selection::{SelectionManager, TokenSelection, ImplicitSelection};
...
impl DatasetSelector {
    ...
    fn is_terminal_available() -> bool {
        ...
        if let Ok((cols, rows)) = terminal::size() {
            if cols > 0 && rows > 0 {
                debug!("Terminal size available: {}x{}, assuming terminal is available", cols, rows);
                return true;
            }
        }
    ...
    fn organize_datasets(datasets: Vec<DiscoveredDataset>) -> Vec<DatasetItem> {
        ...
        let mut folders: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
        
        // Group by folder and count
        for dataset in &datasets {
            let folder_name = dataset.dataset_info.name.split('/').next().unwrap_or(&dataset.dataset_info.name).to_string();
            *folders.entry(folder_name).or_insert(0) += 1;
        }
        
        // Add folder headers
        for (folder_name, count) in folders {
            items.push(DatasetItem::Folder { name: folder_name.clone(), datasets_count: count });
            
            // Add datasets in this folder
            for dataset in &datasets {
                let dataset_folder = dataset.dataset_info.name.split('/').next().unwrap_or(&dataset.dataset_info.name);
    ...
    async fn run_loop(&mut self, terminal: &mut Terminal<CrosstermBackend<Stdout>>) -> Result<DatasetSelectorResult> {
        let mut iteration = 0;
                    ...
                    if let Some(selection_item) = self.selections.get(index) {
                        let tokens = match selection_item {
                            SelectionItem::SavedSelection(selection) => {
                                ...
                                selection.tokens.iter().map(|t| t.token_id.clone()).collect()
                            }
                            SelectionItem::ImplicitSelection(implicit) => {
                                ...
                                let selection_manager = SelectionManager::new(&self.data_paths.data());
    ...
    fn extract_tokens_from_dataset(&self, dataset: &DiscoveredDataset) -> Result<Vec<String>> {
        debug!("Extracting tokens from dataset: {}", dataset.dataset_info.name);
    ...
    fn render_selections_panel(&mut self, f: &mut Frame, area: ratatui::prelude::Rect) {
        ...
        let items: Vec<ListItem> = self.selections.iter().map(|selection| {
            let (name, description) = match selection {
                SelectionItem::SavedSelection(sel) => {
                    (sel.name.clone(), format!("Saved - {} tokens", sel.tokens.len()))
                }
                SelectionItem::ImplicitSelection(imp) => {
                    (imp.name.clone(), format!("Dataset - {} tokens", imp.tokens.len()))
                }
            };
    ...
    fn render_datasets_panel(&mut self, f: &mut Frame, area: ratatui::prelude::Rect) {
        ...
        let items: Vec<ListItem> = self.datasets.iter().map(|dataset| {
            match dataset {
                DatasetItem::Folder { name, datasets_count } => {
                    ListItem::new(vec![
                        Line::from(vec![
                            Span::styled(format!("📂 {}", name), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                        ]),
                        Line::from(vec![
                            Span::styled(format!("   {} datasets", datasets_count), Style::default().fg(Color::Gray)),
                        ])
                    ])
                }
                DatasetItem::Dataset(dataset) => {
                    let short_name = dataset.dataset_info.name.split('/').last().unwrap_or(&dataset.dataset_info.name);
                    ListItem::new(vec![
                        Line::from(vec![
                            Span::styled(format!("  📄 {}", short_name), Style::default().fg(Color::White)),
                        ]),
                        Line::from(vec![
                            Span::styled(format!("     {} files", dataset.dataset_info.file_count), Style::default().fg(Color::Gray)),
                        ])
                    ])
                }
            }
