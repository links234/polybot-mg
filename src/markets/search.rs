...
use super::display::{display_market_info, display_gamma_market_info};
...
pub async fn search_markets(
    ...
    println!("{}", format!("🔍 Searching for markets matching '{}'...", keyword).bright_blue());
...
pub async fn get_market_details(
    ...
    println!("{}", format!("📊 Fetching market details for '{}'...", identifier).bright_blue());
    ...
    match market_response {
        ...
        Err(e) => {
            println!("{}", format!("Could not find market in CLOB API: {}", e).yellow());
        ...
        if let Some(clob_token_ids) = market.get("clob_token_ids").and_then(|v| v.as_array()) {
            ...
            for (idx, token_id) in clob_token_ids.iter().enumerate() {
                if let Some(id) = token_id.as_str() {
                    println!("  {} Token {}: {}", "→".bright_black(), idx, id.bright_yellow());
                }
            }
        }
    } else {
...
pub async fn get_market_from_url(url: &str) -> Result<()> {
    println!("{}", format!("🌐 Fetching market information from URL: {}", url).bright_blue());
    ...
    if let Some(event) = matching_event {
        ...
        if let Some(markets) = event.get("markets").and_then(|v| v.as_array()) {
            ...
            for (idx, market) in markets.iter().enumerate() {
                if let Some(market_id) = market.get("id").and_then(|v| v.as_i64()) {
                    ...
                    let slug = market.get("slug").and_then(|v| v.as_str()).unwrap_or("unknown");
                    
                    println!("\n{}. {} (ID: {})", idx + 1, title.bright_white(), market_id);
                    println!("   Slug: {}", slug);
                    ...
                    if let Some(clob_token_ids) = market.get("clob_token_ids").and_then(|v| v.as_array()) {
                        ...
                        for (token_idx, token_id) in clob_token_ids.iter().enumerate() {
                            if let Some(id) = token_id.as_str() {
                                println!("     {} Token {}: {}", "→".bright_black(), token_idx, id.bright_yellow());
                            }
                        }
                    }
                }
        ...
        let related_markets: Vec<_> = all_markets.iter()
            .filter(|m| {
                let title = m.get("title").and_then(|v| v.as_str()).unwrap_or("").to_lowercase();
                let market_slug = m.get("slug").and_then(|v| v.as_str()).unwrap_or("").to_lowercase();
