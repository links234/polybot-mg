...
impl MarketEnricher {
    ...
    async fn load_source_markets(&self, source_path: &Path) -> Result<Vec<Market>> {
        ...
        let markets: Vec<Market> = values.into_iter()
    ...
    /// Calculate orderbook metrics
    fn calculate_orderbook_metrics(orderbook: &polymarket_rs_client::OrderBookSummary) -> Option<OrderbookMetrics> {
    ...
    /// Calculate liquidity metrics
    fn calculate_liquidity_metrics(orderbook: &polymarket_rs_client::OrderBookSummary) -> LiquidityMetrics {
    ...
    fn display_results(&self, total_markets: usize, enriched_markets: &[EnrichedMarket], statistics: &EnrichmentStatistics) {
        ...
        info!("Markets with orderbook: {}", statistics.markets_with_orderbook);
        info!("Markets with liquidity data: {}", statistics.markets_with_liquidity);
        info!("Markets with volume data: {}", statistics.markets_with_volume);
        
        if statistics.average_spread_percentage > 0.0 {
            info!("Average spread: {:.2}%", statistics.average_spread_percentage);
        }
        
        info!("Average processing time: {:.1}ms", statistics.average_processing_time_ms);
