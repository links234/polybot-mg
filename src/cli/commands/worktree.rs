...
pub async fn worktree(args: WorktreeArgs, _host: &str, data_paths: DataPaths) -> Result<()> {
    match args.command {
        ...
        WorktreeCommand::List => list_worktrees().await,
        WorktreeCommand::Remove { path, force } => remove_worktree(&path, force).await,
...
async fn create_worktree(
    ...
    if copy_data {
        ...
        if source_data.exists() {
            ...
            warn!(
                "Source data directory doesn't exist: {}",
                source_data.display()
            );
        }
    }
...
fn print_worktree_info(info: &WorktreeInfo) -> Result<()> {
    ...
    let is_main = info
        .branch
        .as_ref()
        .map_or(false, |b| b.ends_with("/main") || b.ends_with("/master"));
...
async fn sync_worktree(source: Option<&str>, what: &str, _data_paths: &DataPaths) -> Result<()> {
    ...
    match what {
        ...
        _ => {
            return Err(anyhow!(
                "Unknown sync type: {}. Use: data, env, creds, or all",
                what
            ));
        }
    }
...
fn copy_env_files(target_dir: &Path) -> Result<()> {
    ...
    for env_file in &env_files {
        ...
        if source.exists() {
            fs::copy(&source, &target).context(format!("Failed to copy {}", env_file))?;
...
fn copy_credentials(source_auth: &Path, target_auth: &Path) -> Result<()> {
    if !source_auth.exists() {
        warn!(
            "Source auth directory doesn't exist: {}",
            source_auth.display()
        );
        return Ok(());
    }

    copy_directory_recursive(source_auth, target_auth).context("Failed to copy credentials")?;
...
"#,
    ...
    fs::write(readme_path, readme_content).context("Failed to create worktree README")?;
