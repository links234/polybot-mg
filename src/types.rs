...
mod tests {
    ...
    fn test_order_book_levels() {
        let bids = vec![PriceLevel::new(dec!(0.4), dec!(100.0))];
        let asks = vec![PriceLevel::new(dec!(0.6), dec!(200.0))];
        let levels = OrderBookLevels::new(bids, asks);
        
        assert_eq!(levels.depth().bid_levels, 1);
        assert_eq!(levels.depth().ask_levels, 1);
        assert_eq!(levels.best_bid().unwrap().price, dec!(0.4));
        assert_eq!(levels.best_ask().unwrap().price, dec!(0.6));
    }
