...
fn draw_ui(f: &mut Frame, state: &PortfolioViewState) {
    ...
    let user_info = vec![
        ...
        Line::from(vec![
            Span::raw("🔗 Profile: "),
            Span::styled(&state.profile_url, Style::default().fg(Color::Blue).add_modifier(Modifier::UNDERLINED)),
        ]),
    ];
    
    let header_left = Paragraph::new(user_info)
        .block(Block::default().borders(Borders::ALL).title("Portfolio Overview"));
    f.render_widget(header_left, header_chunks[0]);
    
    // Account summary
    let total_order_value: Decimal = state.orders.iter()
    ...
    let summary = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{:.2}", total_order_value), Style::default().fg(Color::Green)),
            Span::raw(" USDC"),
        ]),
    ];
    
    let header_right = Paragraph::new(summary)
        .block(Block::default().borders(Borders::ALL).title("Account Summary"));
...
fn draw_orders_table(f: &mut Frame, area: Rect, state: &PortfolioViewState) {
    ...
    let rows: Vec<Row> = state.orders.iter().enumerate().map(|(i, order)| {
...
fn draw_positions_table(f: &mut Frame, area: Rect, state: &PortfolioViewState) {
    if state.positions.is_empty() {
        let empty_msg = Paragraph::new("No positions found. Positions will appear here once orders are filled.")
            .style(Style::default().fg(Color::DarkGray))
            .alignment(Alignment::Center)
            .block(Block::default()
                .borders(Borders::ALL)
    ...
    let rows: Vec<Row> = state.positions.iter().enumerate().map(|(i, pos)| {
        ...
        let pnl_percent_cell = if pos.pnl_percent >= Decimal::ZERO {
            Cell::from(format!("+{:.2}%", pos.pnl_percent)).style(Style::default().fg(Color::Green))
        } else {
            Cell::from(format!("{:.2}%", pos.pnl_percent)).style(Style::default().fg(Color::Red))
        };
