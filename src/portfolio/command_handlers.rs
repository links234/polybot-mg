...
use crate::portfolio::{PortfolioServiceHandle, start_portfolio_service};
...
impl PortfolioCommandHandlers {
    ...
    pub async fn new(host: String, data_paths: DataPaths) -> Result<Self> {
        ...
        let service_handle = get_or_start_portfolio_service(data_paths, address.clone(), host.clone()).await?;
    ...
    pub async fn execute_buy(
        ...
        info!("Placing buy order: token={}, price={}, size={}", token_id, price, size);
        ...
        let order_id = self.service_handle.buy(
            market_id,
            token_id.to_string(),
            price,
            size,
        ...
        if let Err(e) = self.service_handle.refresh_data().await {
            warn!("Failed to refresh portfolio data after buy order: {}", e);
        }

        // Create snapshot after trade
        if let Err(e) = self.service_handle.create_snapshot(format!("buy_order_{}", order_id)).await {
    ...
    pub async fn execute_sell(
        ...
        info!("Placing sell order: token={}, price={}, size={}", token_id, price, size);

        // Use market_id if provided, otherwise use token_id as fallback
        let market_id = market_id.unwrap_or_else(|| token_id.to_string());

        // Execute sell order through portfolio service
        let order_id = self.service_handle.sell(
            market_id,
            token_id.to_string(),
            price,
            size,
        ...
        // Create snapshot after trade
        if let Err(e) = self.service_handle.create_snapshot(format!("sell_order_{}", order_id)).await {
    ...
    pub async fn execute_cancel(&self, order_id: &str) -> Result<bool> {
        ...
        if success {
            ...
            if let Err(e) = self.service_handle.create_snapshot(format!("cancel_order_{}", order_id)).await {
    ...
    pub async fn get_portfolio_state(&self) -> Result<crate::portfolio::PortfolioState> {
        self.service_handle.get_portfolio_state().await
    }

    /// Get active orders
    pub async fn get_active_orders(&self) -> Result<Vec<crate::portfolio::ActiveOrder>> {
    ...
    ) -> Result<Vec<crate::portfolio::TradeExecution>> {
        self.service_handle.get_trade_history(start_date, end_date).await
    }
...
pub async fn enhanced_buy_command(
    ...
    let order_id = handlers.execute_buy(token_id, price, size, market_id, confirm).await?;
...
pub async fn enhanced_sell_command(
    ...
    let handlers = PortfolioCommandHandlers::new(host.to_string(), data_paths).await?;
    let order_id = handlers.execute_sell(token_id, price, size, market_id, confirm).await?;
...
pub async fn enhanced_cancel_command(
    order_id: &str,
    host: &str,
    data_paths: DataPaths,
) -> Result<()> {
...
pub async fn enhanced_portfolio_command(
    ...
    println!("🔗 Profile: https://polymarket.com/profile/{}", handlers.get_address());
    println!("🌐 API Host: {}", handlers.get_host());
    println!("🔄 Last Updated: {}", portfolio_state.last_updated.format("%Y-%m-%d %H:%M:%S UTC"));
    println!("✅ Synced: {}", if portfolio_state.is_synced { "Yes" } else { "No" });
    println!();
    ...
    println!("  🔒 Locked in Orders: ${:.2}", portfolio_state.balances.locked_in_orders);
    println!("  📈 Position Value: ${:.2}", portfolio_state.balances.position_value);
    println!();
    
    // Display statistics
    println!("📊 Portfolio Statistics:");
    println!("  🎯 Total Positions: {}", portfolio_state.stats.total_positions);
    println!("  📈 Open Positions: {}", portfolio_state.stats.open_positions);
    println!("  💹 Total P&L: ${:.2}", portfolio_state.stats.total_pnl());
    println!("  ✅ Realized P&L: ${:.2}", portfolio_state.stats.total_realized_pnl);
    println!("  📊 Unrealized P&L: ${:.2}", portfolio_state.stats.total_unrealized_pnl);
    println!("  💸 Total Fees: ${:.2}", portfolio_state.stats.total_fees_paid);
    ...
    if !positions.is_empty() {
        ...
        for (i, position) in positions.iter().enumerate() {
            println!("  {}. {} {} - Size: {}, Avg Price: ${:.3}, P&L: ${:.2}",
                i + 1,
                position.outcome,
