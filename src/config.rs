use aes_gcm::{
    aead::{Aead, KeyInit, rand_core::{RngCore, OsRng}},
    Aes256Gcm, Nonce, Key
};
...
async fn load_stored_credentials(creds_path: &PathBuf, passphrase: &str) -> Result<StoredCredentials> {
    // Read encrypted file
    let encrypted = std::fs::read(creds_path)?;
    
    if encrypted.len() < 28 { // 16 (salt) + 12 (nonce) = 28
...
async fn save_stored_credentials(creds_path: &PathBuf, passphrase: &str, stored: &StoredCredentials) -> Result<()> {
