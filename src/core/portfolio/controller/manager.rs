...
impl PortfolioManager {
    ...
    pub async fn update_stats(&self) -> Result<()> {
        ...
        // Calculate P&L
        stats.total_realized_pnl = positions.values().map(|p| p.realized_pnl).sum();

        stats.total_unrealized_pnl = positions.values().filter_map(|p| p.unrealized_pnl).sum();

        stats.total_fees_paid = positions.values().map(|p| p.fees_paid).sum();
        ...
        if !closed_positions.is_empty() {
            ...
            let winning_trades: Vec<_> = closed_positions
                .iter()
                .filter(|p| p.realized_pnl > Decimal::ZERO)
                .collect();

            let losing_trades: Vec<_> = closed_positions
                .iter()
                .filter(|p| p.realized_pnl < Decimal::ZERO)
                .collect();
    ...
    pub async fn get_market_positions(&self) -> Vec<MarketPositionSummary> {
        ...
        for (market_id, positions) in market_positions {
            let market_orders: Vec<_> = orders
                .values()
                .filter(|o| o.market_id == market_id)
                .collect();

            let total_exposure = positions.iter().map(|p| p.size * p.average_price).sum();

            let net_position = positions
                .iter()
                .map(|p| match p.side {
            ...
            let total_pnl = positions.iter().map(|p| p.total_pnl()).sum();

            let market_question = market_info
                .get(&market_id)
    ...
    /// Get active orders synchronously for TUI (non-blocking)
    pub fn get_active_orders_sync(&self) -> Vec<ActiveOrder> {
        if let Ok(orders) = self.active_orders.try_read() {
            orders.values().cloned().collect()
        } else {
            Vec::new()
        }
    ...
    /// Get portfolio statistics synchronously for TUI (non-blocking)
    ...
    /// Public access to positions for TUI
    pub fn positions(&self) -> &Arc<RwLock<HashMap<String, Position>>> {
        &self.positions
    }

    /// Public access to active orders for TUI  
    pub fn active_orders(&self) -> &Arc<RwLock<HashMap<String, ActiveOrder>>> {
        &self.active_orders
    }

    /// Public access to stats for TUI
    pub fn stats(&self) -> &Arc<RwLock<PortfolioStats>> {
        &self.stats
    }
}
