...
use crate::typed_store::{
    ...
    models::{
        ...
        MarketTable, MarketByConditionTable, ConditionTable, 
        TokensByConditionTable, MarketIndexTable,
        MarketCf, MarketByConditionCf, ConditionCf,
...
impl IndexCommand {
    ...
    async fn execute_file_store(&self, data_paths: &DataPaths) -> Result<()> {
        ...
        for (i, chunk_file) in chunk_files.iter().enumerate() {
            info!("🔄 Processing chunk {}/{}: {}", i + 1, chunk_files.len(), chunk_file.display());
            
            let content = fs::read_to_string(chunk_file)?;
            
            // Parse markets from various formats
            let markets = if let Ok(market_array) = serde_json::from_str::<Vec<serde_json::Value>>(&content) {
                market_array
            ...
            for market_value in markets {
                match Market::from_value(market_value) {
                    Ok(market) => {
                        ...
                        if market.condition_id.as_ref().map_or(true, |id| id.trim().is_empty()) {
                            skipped_markets += 1;
                            continue;
                        }
    ...
    async fn execute_rocksdb_typed(&self, data_paths: &DataPaths) -> Result<()> {
        ...
        if self.progress_sender.is_none() {
            info!("🚀 Using {} threads for parallel processing", rayon::current_num_threads());
            info!("🗄️ Starting market data indexing to RocksDB");
        }
        
        // Send initial progress update
        if let Some(ref sender) = self.progress_sender {
            let _ = sender.send(ProgressUpdate::PhaseChange(crate::tui::IndexingPhase::Starting));
            let _ = sender.send(ProgressUpdate::Event("🗄️ Starting market data indexing to RocksDB".to_string()));
        }
        ...
        if self.progress_sender.is_none() {
            ...
            let _ = sender.send(ProgressUpdate::Event(format!("📂 Database path: {}", db_path.display())));
        }
        ...
        if self.args.clear && db_path.exists() {
            if self.progress_sender.is_none() {
                ...
                let _ = sender.send(ProgressUpdate::Event("🗑️ Clearing existing database".to_string()));
            }
            fs::remove_dir_all(&db_path)?;
        }
        ...
        if self.progress_sender.is_none() {
            ...
            let _ = sender.send(ProgressUpdate::Event("✅ Opened TypedDbContext database".to_string()));
        }
        ...
        if chunk_files.is_empty() {
            if self.progress_sender.is_none() {
                ...
                let _ = sender.send(ProgressUpdate::Error("No market chunk files found to index".to_string()));
            }
            return Ok(());
        }
        ...
        if let Some(ref sender) = self.progress_sender {
            let _ = sender.send(ProgressUpdate::PhaseChange(crate::tui::IndexingPhase::ProcessingFiles));
            let _ = sender.send(ProgressUpdate::Event(format!("📋 Found {} chunk files to index", chunk_files.len())));
        }
        ...
        let conditions_map = Arc::try_unwrap(conditions_map).unwrap().into_inner().unwrap();
        let tokens_by_condition = Arc::try_unwrap(tokens_by_condition).unwrap().into_inner().unwrap();
        ...
        if let Some(ref sender) = self.progress_sender {
            let _ = sender.send(ProgressUpdate::PhaseChange(crate::tui::IndexingPhase::IndexingConditions));
        ...
        if let Some(ref sender) = self.progress_sender {
            let _ = sender.send(ProgressUpdate::PhaseChange(crate::tui::IndexingPhase::IndexingTokens));
        ...
        if let Some(ref sender) = self.progress_sender {
            let _ = sender.send(ProgressUpdate::PhaseChange(crate::tui::IndexingPhase::Finalizing));
        }
        ...
        if self.progress_sender.is_none() {
            ...
            info!("   • Database size: ~{:.2} MB", self.estimate_db_size(&db_path)?);
        }
        
        // Send completion
        if let Some(ref sender) = self.progress_sender {
            let _ = sender.send(ProgressUpdate::PhaseChange(crate::tui::IndexingPhase::Completed));
            let _ = sender.send(ProgressUpdate::Event(format!("✅ Indexed {} markets, {} conditions, {} tokens", 
    ...
    async fn execute_rocksdb_legacy(&self, data_paths: &DataPaths) -> Result<()> {
        ...
        for (i, chunk_file) in chunk_files.iter().enumerate() {
            ...
            let chunk_result = self.process_chunk(
                &store,
                chunk_file,
                &mut conditions_map,
    ...
    fn get_chunk_files(&self, data_paths: &DataPaths) -> Result<Vec<PathBuf>> {
        ...
        if let Some(ref chunk_files_str) = self.args.chunk_files {
            ...
            let source_dir = self.args.source_dir
                .clone()
    ...
    async fn process_chunk(
        ...
        for market_value in markets {
            ...
            if let Some(condition) = rocks_market.extract_condition() {
                ...
                conditions_map.entry(condition.id.clone())
            ...
            for token in tokens {
                if let Some(cond_id) = &token.condition_id {
                    tokens_by_condition.entry(cond_id.clone())
    ...
    fn process_chunk_parallel(
        ...
        if let Some(ref sender) = progress_sender {
            let _ = sender.send(ProgressUpdate::Event(format!("📄 Processing file: {}", chunk_file.file_name().unwrap_or_default().to_string_lossy())));
        }
        let content = fs::read_to_string(chunk_file)?;
        ...
        if let Some(ref sender) = progress_sender {
            let _ = sender.send(ProgressUpdate::Event(format!("📊 Parsed {} markets from file", markets.len())));
        }
        ...
        for (chunk_idx, chunk) in markets.chunks(chunk_size).enumerate() {
            ...
            if let Some(ref sender) = progress_sender {
                let _ = sender.send(ProgressUpdate::Event(format!("🔄 Processing chunk {} with {} markets", chunk_idx + 1, chunk.len())));
            }
            ...
            let results: Vec<_> = chunk
                ...
                .filter_map(|market_value| {
                    ...
                    Some(Ok((market_id, condition_id, rocks_market, condition, tokens, index)))
                })
                .collect();
                ...
                match result {
                    Ok((market_id, condition_id, rocks_market, condition, tokens, index)) => {
                        ...
                        if let Some(cond) = condition {
                            ...
                            conditions.entry(cond.id.clone())
                                .and_modify(|existing| existing.market_count += 1)
                                .or_insert(cond);
                        }
                        ...
                        for token in tokens {
                            if let Some(cond_id) = &token.condition_id {
                                tokens_map.entry(cond_id.clone())
                                    .or_insert_with(Vec::new)
                                    .push(token);
                            }
                        }
            ...
            if !batch_markets.is_empty() {
                // Send progress update via channel instead of console logging
                if let Some(ref sender) = progress_sender {
                    let _ = sender.send(ProgressUpdate::Event(format!("📝 Writing batch of {} markets to database", batch_markets.len())));
                }
                ctx.batch_write(|batch| {
