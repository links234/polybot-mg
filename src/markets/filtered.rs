...
use super::cache::{MarketCache, fetch_and_cache_markets};
...
pub async fn list_filtered_markets(
    ...
    let cache = if refresh {
        ...
        match MarketCache::load()? {
            Some(cache) => {
                println!("{}", format!("📂 Using cached data from {}", cache.last_updated.format("%Y-%m-%d %H:%M:%S UTC")).bright_cyan());
                cache
            }
            None => {
                println!("{}", "📂 Cache not found or expired, fetching fresh data...".bright_yellow());
                fetch_and_cache_markets(&client).await?
            }
        }
    };
    ...
    let title = if min_price.is_some() || max_price.is_some() {
        ...
        format!("Top {} Binary Markets by Volume (Price Range: {}%-{}%)", display_markets.len(), min_p, max_p)
    } else {
    ...
    if detailed {
        ...
        for (idx, market) in display_markets.iter().enumerate() {
            ...
            println!("   {} {}", "Condition ID:".bright_black(), market.condition_id.bright_yellow());
