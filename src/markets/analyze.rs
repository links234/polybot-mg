...
impl MarketAnalyzer {
    ...
    pub async fn execute(&self, data_paths: &DataPaths) -> Result<AnalysisResult> {
        ...
        info!("🔍 Analyzing markets from dataset '{}'...", self.source_dataset);
    ...
    async fn load_markets(&self, source_path: &Path) -> Result<Vec<Value>> {
        ...
        for file_path in data_files {
            debug!("Processing: {}", file_path.file_name().unwrap().to_string_lossy());
    ...
    fn apply_filters(&self, markets: &[Value]) -> Result<Vec<Value>> {
        ...
        info!("📊 Loaded {} markets, filtered to {}", total, filtered.len());
        Ok(filtered)
    }
