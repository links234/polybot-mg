...
pub async fn show_orderbook(client: ClobClient, token_id: &str, depth: usize) -> Result<()> {
    println!(
        "{}",
    ...
    let mut relevant_bids: Vec<_> = orderbook
        .bids
        .iter()
    ...
    let mut relevant_asks: Vec<_> = orderbook
        .asks
        .iter()
    ...
    if !relevant_bids.is_empty() && !relevant_asks.is_empty() {
        ...
        println!("{} ${:.4}", "Mid Price:".bright_white(), mid_price);
    }
    ...
    if relevant_bids.is_empty() {
        println!(
            "{}",
            "No relevant bids near market price".italic().bright_black()
        );
    } else {
        println!(
            "{:>10} {:>15}",
    ...
    if relevant_asks.is_empty() {
        println!(
            "{}",
            "No relevant asks near market price".italic().bright_black()
        );
    } else {
        println!(
            "{:>10} {:>15}",
    ...
    if !relevant_bids.is_empty() || !relevant_asks.is_empty() {
        ...
        let total_bid_size: Decimal = relevant_bids.iter().map(|b| b.size).sum();
        let total_ask_size: Decimal = relevant_asks.iter().map(|a| a.size).sum();
