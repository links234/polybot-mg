...
impl FetchAllMarketsCommand {
    ...
    pub async fn execute(&self, host: &str, data_paths: DataPaths, verbose: bool) -> Result<()> {
        ...
        let force_refresh = self.args.force_refresh || 
            std::env::var("FORCE_REFRESH").unwrap_or_default().to_lowercase() == "true";
    ...
    async fn check_cache_validity(&self, dataset_path: &std::path::Path, verbose: bool) -> Result<bool> {
        if !dataset_path.exists() {
            if verbose {
                warn!("📂 Dataset directory '{}' does not exist", dataset_path.display());
            }
            return Ok(false);
        }
        ...
        if verbose {
            ...
            info!("🕒 Cache check for '{}':", dataset_path.file_name().unwrap_or_default().to_string_lossy());
            info!("   Created: {} ({})", created_at.format("%Y-%m-%d %H:%M:%S UTC"), self.format_duration(age));
            info!("   Expires: {} ({})", expires_at.format("%Y-%m-%d %H:%M:%S UTC"), 
                if is_valid { 
