...
use crate::core::portfolio::api::PortfolioState;
...
pub struct DashboardFormatter<'a> {
    pub portfolio_state: &'a PortfolioState,
    pub address: &'a str,
    pub host: &'a str,
}

impl<'a> DashboardFormatter<'a> {
    pub fn new(portfolio_state: &'a PortfolioState, address: &'a str, host: &'a str) -> Self {
        Self {
            portfolio_state,
            address,
            host,
    ...
    pub fn format_dashboard(&self) -> String {
        ...
        output.push_str(&format!(
            "✅ Synced: {}\n\n",
            if self.portfolio_state.is_synced {
                "Yes"
            } else {
                "No"
            }
