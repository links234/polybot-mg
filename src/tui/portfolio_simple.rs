...
pub fn display_portfolio_simple(user_address: &str, orders: &[PolymarketOrder]) -> Result<()> {
    ...
    println!("🔗 Profile: {}", format!("https://polymarket.com/profile/{}", user_address).bright_blue().underline());
    
    // Summary section
    let total_order_value: Decimal = orders.iter()
    ...
    println!("💰 Total Order Value: ${:.2} USDC", total_order_value.to_string().bright_green());
    println!("✅ Filled Orders: {}", filled_orders.to_string().bright_blue());
    ...
    println!("{}", "─".repeat(100).bright_black());
    println!("\n{}", "No positions found. Positions will appear here once orders are filled.".bright_black().italic());
    
    // Footer
    println!("\n{}", "─".repeat(100).bright_black());
    println!("{}", "Press Ctrl+C to exit | Use --text flag for simple view".bright_black().italic());
