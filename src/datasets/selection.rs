...
impl SelectionManager {
    ...
    pub fn ensure_directory(&self) -> Result<()> {
        fs::create_dir_all(&self.base_path).context("Failed to create selections directory")?;
        Ok(())
    }
    ...
    fn discover_selections_in_dataset(
        &self,
        dataset_path: &Path,
    ) -> Result<Vec<ImplicitSelection>> {
        let mut selections = Vec::new();
        ...
        self.find_markets_files(dataset_path, &mut |markets_file| {
            if let Ok(tokens) = self.extract_tokens_from_markets_file(markets_file) {
                if !tokens.is_empty() {
                    ...
                    let relative_path = markets_file
                        .strip_prefix(&self.datasets_path)
                        ...
                        return match dataset_type {
                            ...
                            "Pipeline" => {
                                ...
                                if let Some(additional_info) = parsed.get("additional_info") {
                                    if let Some(pipeline_name) = additional_info
                                        .get("pipeline_name")
                                        .and_then(|v| v.as_str())
                                    {
    ...
    pub fn list_selections(&self) -> Result<Vec<String>> {
        ...
        let mut selections = Vec::new();
        let entries =
            fs::read_dir(&self.base_path).context("Failed to read selections directory")?;
    ...
    pub fn load_selection(&self, name: &str) -> Result<TokenSelection> {
        ...
        if explicit_path.exists() {
            let content = fs::read_to_string(&explicit_path).with_context(|| {
                format!("Failed to read selection file: {}", explicit_path.display())
            })?;
