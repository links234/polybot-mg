...
impl SelectionBuilder {
    ...
    async fn discover_tokens() -> Result<Vec<TokenOption>> {
        ...
        for dataset in datasets {
            ...
            if matches!(dataset.dataset_info.dataset_type, DatasetType::Pipeline { .. }) {
                continue;
            }
            ...
            // Avoid duplicates
            if !tokens.iter().any(|t: &TokenOption| t.token_id == token_option.token_id) {
    ...
    async fn run_loop(&mut self, terminal: &mut Terminal<CrosstermBackend<Stdout>>) -> Result<SelectionBuilderResult> {
        loop {
    ...
    fn render_token_selection(&mut self, f: &mut Frame, area: ratatui::prelude::Rect) {
        ...
        let items: Vec<ListItem> = self.filtered_indices
            ...
            .map(|&idx| {
                ...
                let token_display = if token.token_id.len() > 20 {
                    format!("{}...{}", &token.token_id[..10], &token.token_id[token.token_id.len()-10..])
                } else {
                ...
                if let Some(market) = &token.market_info {
                    ...
                    spans.push(Span::styled(market_display, Style::default().fg(Color::Gray)));
                }
        ...
        let list = List::new(items)
            .block(Block::default()
                .title(format!("Available Tokens ({} found, {} selected)", 
        ...
        let summary_text = if self.selected_tokens.is_empty() {
            ...
            if self.selected_tokens.len() > 5 {
                lines.push(Line::from(format!("... and {} more", self.selected_tokens.len() - 5)));
            }
        ...
        if !self.search_filter.is_empty() {
            ...
            let search = Paragraph::new(search_display)
    ...
    fn render_confirmation(&self, f: &mut Frame, area: ratatui::prelude::Rect) {
        ...
        let mut lines = vec![
            ...
            Line::from(vec![
                Span::raw("Name: "),
                Span::styled(&self.name, Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
        ];
